...
//});
	...
	$( ".top-menu" ).click(function(res) {
		...
		if(res.target.innerHTML=="Request Approval"){
			$.ajax({
				type:'POST',
				url:$('.base').val()+"public/EmpAllotment/approveRequest.php",	
				data:{empID:$(".user-id")[0].innerText,empName:$(".user-name")[0].innerText,count:1},
...
function logitOut(){
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/jms/calcProdIdleBreak.php",
...
function closeopWiseMod(ref){
	...
	if(ref.defaultValue=='Day Signout'){
		...
		$.ajax({
		type:'POST',
		url:$('.base').val()+"public/jms/punctuality.php",
		data:{empID:$(".user-id")[0].innerText,empName:$(".user-name")[0].innerText},
		success:function(html){
			var c =html.split('.')[1];
			if(c=="reached"){
...
function lowPriorityFiles(){
	...
	for(i=0;i<filesList.length;i++){
		$.ajax({
		type:'POST',
		url:$('.base').val()+"public/jms/moveLowPriorFiles.php",
...
function monthsDate(date,id){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/CapacityPlanner/getDatesInMonth.php",	
		data:{date1:date},
		success:function(html){
			var cc = html.split("_");
			for(i=0;i<cc.length;i++){
					$.ajax({
						type:'POST',
						url:$('.base').val()+"public/CapacityPlanner/getOTDmonth.php",	
						data:{date1:cc[i]},
						success:function(html){
							var c =html.split("_");
		...
		if (confirm("Are you sure you want to re-assign complexity for already assigned file?.")) {
			...
			for(i=0;i<re.length;i++){
				if(re[i].checked==true){
					...
					$.ajax({
							type:'POST',
							url:$('.base').val()+"public/CapacityPlanner/vars4.php",	
...
function getQualityAnalysis(){
	...
	$("#QualAn").html("100% Quality Acheived");
		...
		function chartingFn(){
			...
			$.ajax({
				type:'POST',
				url:$('.base').val()+"public/CapacityPlanner/getChartForDate.php",	
		...
		function getfromchartDisplay(){
			$.ajax({
							type:'POST',
							url:$('.base').val()+"public/Reports/opWiseSheet.php",	
...
function opInfo(retthis){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/CapacityPlanner/opInfo.php",	
...
function reloadimage(img){
	...
	img.src = $('.base').val()+"img/dmg/"+filename;//	$("#"+img.id)[0].src = "http://10.98.2.46:81/FinalCopy/img/dmg/"+filename; //http://10.98.2.46:81/FinalCopy/
		...
		for(si=0;si<totalName.length;si++){
			for(sj=0;sj<tdlen.length;sj++){
				if(totalName[si]==(tdlen[sj].innerText).replace(/(\n)/gm,"")){
					...
					src.outerHTML="<img class='zoomImage' style='width:"+src.style.width+";height:"+src.style.height+";' src='"+$('.base').val()+"img/dmg/"+(tdlen[sj].innerText).replace(/(\n)/gm,"") +"' onclick='reloadimage(this);' title='Click to reload'>";
				}
			}
	...
	if(lenmulti>=1){
		if(confirm("Are you sure you want to close the job?")){	
			for(m=0;m<lenmulti;m++){
				...
				$.ajax({
					type:'POST',
					url:$('.base').val()+"public/CapacityPlanner/vars9.php",
...
function download_csv(tableID, filename) {
	//window.open('data:application/vnd.ms-excel,' + $('#'+tableID)[0].childNodes[1].outerHTML);
...
function export_table_to_csv(rows, filename) {
	...
	var joinedcsv = csv.join("\n");
	var csvFile;
    var downloadLink;

    // CSV FILE
    csvFile = new Blob([joinedcsv], {type: "text/csv"});
