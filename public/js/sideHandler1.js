		...
		if(totalJob.length>0){
			...
			if(complex==totalJob.length){
				for(n=0;n<totalJob.length;n++){
					...
					$.ajax({
						type:'POST',
						url:$('.base').val()+"public/CapacityPlanner/vars3.php",	
						...
						complete: function(html){
							if(assignedYess[incF1]=="no"){
								...
								$.ajax({
									type:'POST',
									url:$('.base').val()+"public/CapacityPlanner/vars.php",	
									data:{File_Name:(fileNmaeArr[incF1]).replace('\n',''),filHan:$(".user-name")[0].innerText,curtime:strTime,singORMult:"single",complexity:siblingArr[incF1],folder:fName,lpOrNot:lpOrNot,pubDate:pubDate},
									success:function(html){
										var c = html;
		...
		complete: function(html){
			if(assignedYes=="no"){
				...
				$.ajax({
					type:'POST',
					url:$('.base').val()+"public/CapacityPlanner/vars.php",	
					data:{File_Name:(fileName).replace('\n',''),filHan:$(".user-name")[0].innerText,curtime:strTime,singORMult:"single",complexity:cmplxty,folder:fName,lpOrNot:lpOrNot,pubDate:pubDate},
					success:function(html){
						closeModals();
						//sample(html);
						if(mulArgs=='single'){
...
function getfileForQA(){
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/QA/getfilesForQA.php",	
...
function assignQAFiles(ret){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/CapacityPlanner/assignfilesForQA.php",	
		data:{assignedTime:curTimes,filename:ret.innerHTML,qcname:$(".user-name")[0].innerText,forQC:'false'},
		success:function(html){
			var c= html;
...
function startAux(re){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/jms/auxTrack.php",
		data:{team:t[Number($("#selectTeamName")[0].selectedIndex)].label,publisher:p[Number($("#selectPublisherName")[0].selectedIndex)].label,magazine:m[Number($("#selectMagazineName")[0].selectedIndex)].label,OpName:$(".user-name")[0].innerText,type:'',timeLabel:'start',fileName:$("#folderName")[0].value,startimes:startime,empID:$(".user-id")[0].innerText},
		success:function(html){
		   var c = html;
...
function pauseAux(re){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/jms/auxTrack.php",
		data:{team:t[Number($("#selectTeamName")[0].selectedIndex)].label,publisher:p[Number($("#selectPublisherName")[0].selectedIndex)].label,magazine:m[Number($("#selectMagazineName")[0].selectedIndex)].label,OpName:$(".user-name")[0].innerText,type:'',timeLabel:'pause',fileName:$("#folderName")[0].value,pausetimes:pausetime,empID:$(".user-id")[0].innerText},
		success:function(html){
		   var c = html;
...
function resumeAux(re){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/jms/auxTrack.php",
		data:{team:t[Number($("#selectTeamName")[0].selectedIndex)].label,publisher:p[Number($("#selectPublisherName")[0].selectedIndex)].label,magazine:m[Number($("#selectMagazineName")[0].selectedIndex)].label,OpName:$(".user-name")[0].innerText,type:'',timeLabel:'resume',fileName:$("#folderName")[0].value,resumetimes:resumetime,empID:$(".user-id")[0].innerText},
		success:function(html){
		   var c = html;
...
function endAux(re){
	...
	if(sAux || mAux || cAux){
		if((Number(sAux) +Number(mAux)+Number(cAux))>0){
			...
			$.ajax({
				type:'POST',
				url:$('.base').val()+"public/CapacityPlanner/auxTrack.php",
				data:{timeLabel:'end',OpName:$(".user-name")[0].innerText,fileName:$("#folderName")[0].value,endtimes:endtime,s:sAux,m:mAux,c:cAux},
				success:function(html){
				   var c = html;
...
function dispPub(ret){
	...
	$.ajax({
		type:'POST',
		url:$('.base').val()+"public/CapacityPlanner/ajaxData.php",
